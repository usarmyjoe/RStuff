...
```{r echo=FALSE}
barplot(table(ratings),xlab="Rating", ylab="Count", main="Distribution of Ratings")
```


Some movies are also rated more times than others. The plot below shows this clearly. Most movies are rated < 2500 times. But some are clearly rated over that and up to 30000 times. The chart below does not show all movieIds, but is a representative sample.
...
In order to drive the RMSE down, these biases can be used to make better predictions.
...
To make a final prediction, I combined user and movie biases and derived one value of bias:
...
## Results
Each bias and the regularization was used to calculate predictions and return an RMSE (Root Mean Squared Error). These predictions were tracked and compared in the end to select the most accurate RMSE.

```{r echo=FALSE}
kable(RMSE_tracker)
```

Clearly the movie+user effect bias was the winner in predicting the actual ratings from the validation set.
